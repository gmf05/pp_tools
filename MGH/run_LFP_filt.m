...
noise = [0 1e-6 1e-10]; % optimized KS ~ 0.06-0.08
...
% for response = 1
  ...
  m
  m.X = [];
%   m.plot(d,p); plot2svg(['MG49_Seizure45_filt_c' num2str(response)]); pause(0.01); clf;
  m.plot(d,p); pause(0.01); saveas(gcf,[DATA_DIR '/MG49/MG49_filt/MG49_Seizure45_LFP_pp_filt_c' num2str(response) '.fig']);; 
%   ms{response} = m;
end

% save -v7.3 MG49_Seizure45_LFP_pp_filt ms p
