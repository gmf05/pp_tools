	...
	EDF.SIE.RR=1;
        ...
        if EDF.SIE.TECG == 1; 
	        ...
	        if ~isempty(Index)
                ...
                for imk=1:length(EDF.SIE.InChanSelect),
                        k=find(OutChanSelect==EDF.SIE.InChanSelect(imk));
                        if isempty(k)
                                EDF.TECG.QRStemp(:,imk) = zeros(fs,1);
                        else
                                EDF.TECG.QRStemp(:,imk) = QRS.Templates(0.5*fs:1.5*fs-1,k).*hanning(fs);
                        end;
                        [tmp,EDF.TECG.Z(:,imk)] = filter(EDF.TECG.QRStemp(:,imk),1,pulse);
                end;
